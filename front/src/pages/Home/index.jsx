        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6 mt-4 text-center text-lg font-semibold text-orange-500">
          {categoryArr.map(item => (
            <Link
              ...
              className={`${item.idx <= 0 ? 'col-span-2 ' : ''} bg-white shadow-md rounded-lg p-5 lg:p-10`}
              key={item.idx}
            >
              {item.name}
              ...
              <p className="mt-2 lg:mt-6 text-3xl lg:text-5xl">
                <EmojiProvider data={emojiData}>
                  <Emoji name={item.idx !== 0 ? item.icon : item.icon[0]} className="mx-auto w-[40px] lg:w-[65px]" />
                </EmojiProvider>
              </p> */}
            </Link>
          ))}
