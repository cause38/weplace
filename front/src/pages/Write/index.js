...
const Write = () => {
  ...
  const handleStoreSearch = e => {
    ...
    if (searhStoreName.trim() === '') {
      ...
      axios
        .get(`https://dapi.kakao.com/v2/local/search/keyword.json?query=${data}&x=${myLocation.x}&y=${myLocation.y}`, {
  ...
  return (
    <div className="container-wb">
      ...
      <form name="frm">
        ...
        <input type="hidden" value={value.shopIdx || 0} onChange={e => handleFormData('shopIdx', e)} />
        <input type="hidden" value={value.distance || 0} onChange={e => handleFormData('distance', e)} />
        <div className="bg-white p-8 rounded-lg shadow-lg">
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <InputBox label="메뉴명" id="menuName" onChange={e => handleFormData('category', e)} />
              <div>
              ...
              >
                {tagData.map(item => (
                  <span key={item.idx}>
                    ...
                    <input type="checkbox" name="tags" value={item.name} id={`tag_${item.idx}`} className="hidden" />
                  </span>
                ))}
