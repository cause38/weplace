...
const Modal = ({visible, setModalVisible, contents, title, alert}) => {
    useEffect(() => {
        visible
    ...
    return (
        <div className={!visible ? 'hidden' : ''}>
            <div
                className={'fixed top-0 left-0 w-full h-full z-[1000] bg-gray-400 opacity-70'}
                onClick={() => setModalVisible(false)}
            ></div>
            <div
                className={`fixed z-[101] w-11/12 max-w-${
                    alert ? '[400px]' : 'lg'
